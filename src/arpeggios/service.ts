...
export class ArpeggiosService<Response, Payload = Response, IdType = ObjectId> {
  ...
  constructor(prefix: string, config?: ArpeggiosConfig) {
    ...
    this.getById = this.service.getByParam<Response, IdType>(
      this.config?.routes?.getById
    );
    this.deleteAll = this.service.delete<Response[]>(
      this.config?.routes?.deleteAll
    );
    this.deleteById = this.service.deleteByParam<Response, IdType>(
      this.config?.routes?.deleteById
    );
    this.post = this.service.post<Response, Payload>(this.config?.routes?.post);
    this.patch = this.service.patch<Response, Partial<Payload>>(
      this.config.routes?.patch
    );
    this.put = this.service.put<Response, Partial<Payload>>(
